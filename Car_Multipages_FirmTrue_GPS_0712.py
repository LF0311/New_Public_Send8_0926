...
def read_files_split(df):
    ...
    for tme in range(len(df[0])):
        if str('\x00') not in df[13][tme]:
            ...
            n_list.append(float(df[13][tme].split(':')[2])/100)
            e_list.append(float(df[13][tme].split(':')[1])/100)
            id_sent_list.append(id_sent_dic[str(df[1][tme])])
        else:
            time_list.append(
                str((now_time - datetime.timedelta(seconds=(len(df[0]) - tme))).strftime("%Y-%m-%d %H:%M:%S")))
            n_list.append(118.84308210492219)
            e_list.append(31.9784992744323)
            id_sent_list.append(id_sent_dic[str(df[1][tme])])
        ...
        if len(st.session_state.parameters['final_read_file_df']) != 0 \
            ...
            if 55 not in st.session_state.parameters['unique_ids']:
                ...
                if all(value == '无' for value in target_mblb_tmp.values()):
                    ...
                    url = 'http://52.52.52.100:9011/api/WLW_MLFW/sendTargetInfo'
            ...
            for sensID in st.session_state.parameters['unique_ids']:
                if sensID != 55:
                    ...
                    url = 'http://52.52.52.100:9011/api/WLW_MLFW/sendSensorInfo'
...
def app3():
    ...
    # st.set_page_config(page_title='传感器目标识别服务', page_icon=MAGE_EMOJI_URL, initial_sidebar_state='collapsed',
    #                    layout="wide")
